          ...
          {[
            ...
            >
              <div className="flex justify-between items-start">
                <div className="p-3 bg-foreground/5 rounded-xl text-gold">
                  <stat.icon size={24} />
                </div>
                <span className="text-xs font-bold text-foreground/30 uppercase tracking-widest">Real-time</span>
              </div>
              <div className="mt-4">
                <h4 className="text-foreground/40 text-xs font-bold uppercase tracking-widest">{stat.label}</h4>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-sm">
                  {users.slice(0, 5).map((u: any, idx) => (
                    <tr key={idx} className="border-t border-foreground/5 hover:bg-foreground/[0.02] transition-colors group">
                      <td className="p-4 pl-6">
                        <div className="font-bold">{u.name}</div>
                        <div className="text-xs text-foreground/30">{u.email}</div>
                      </td>
                      <td className="p-4 text-foreground/60">{u.shopName || 'N/A'}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-foreground/40 text-xs">{new Date(u.createdAt).toLocaleDateString()}</td>
            ...
            <div className="space-y-6 pt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {packets.slice(0, 4).map((packet: any) => (
                  <motion.div 
                    key={packet._id}
                    className="bg-card glass-card rounded-2xl border border-foreground/5 p-4 flex gap-4 hover:border-gold/30 transition-all"
                  >
                    ...
                    <div className="flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-xs text-foreground/40 line-clamp-1">{packet.description}</p>
                      </div>
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-gold font-black text-xs">₹{packet.price}</span>
                        <span className="text-[8px] bg-foreground/5 px-2 py-0.5 rounded text-foreground/40 uppercase font-black">{packet.category}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
                {packets.length === 0 && (
                  <div className="col-span-full p-8 text-center bg-foreground/5 rounded-2xl border border-dashed border-foreground/10 text-foreground/20 text-xs font-bold uppercase tracking-widest">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button 
                    onClick={() => router.push('/admin/packets')}
                    className="bg-card glass-card p-4 rounded-2xl text-foreground font-bold flex flex-col items-center justify-center gap-2 hover:border-gold/50 border border-foreground/5 transition-all group"
                  >
                  ...
                  <button 
                    ...
                    className="col-span-2 bg-card glass-card p-4 rounded-2xl text-foreground font-bold flex flex-col items-center justify-center gap-2 hover:border-gold/50 border border-foreground/5 transition-all group"
                  >
                ...
                <div className="bg-card glass-card p-6 rounded-2xl space-y-4">
                  <h4 className="text-xs font-bold uppercase tracking-widest text-foreground/40">Quick Stats</h4>
        ...
        <div className="space-y-6 pt-8 border-t border-foreground/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.slice(0, 6).map((t: any) => (
              <motion.div 
                key={t._id}
                className="bg-card glass-card p-6 rounded-2xl border border-foreground/5 relative group hover:border-gold/30 transition-all"
              >
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-12 h-12 rounded-full bg-foreground/5 overflow-hidden shrink-0 border border-foreground/10">
                    {t.avatarUrl ? (
                      <img src={t.avatarUrl} alt={t.userName} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
                        <MessageSquare size={20} className="text-foreground/20" />
                      </div>
                    )}
                  </div>
                  <div>
                ...
                <p className="text-sm text-foreground/60 italic leading-relaxed line-clamp-3">"{t.content}"</p>
            ...
            {testimonials.length === 0 && (
              <div className="col-span-full p-12 text-center bg-foreground/5 rounded-2xl border border-dashed border-foreground/10 text-foreground/20 text-xs font-bold uppercase tracking-widest">
