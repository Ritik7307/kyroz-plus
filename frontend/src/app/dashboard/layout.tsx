...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col relative">
      ...
      {/* Sidebar */}
      <Sidebar isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} userRole="user" />
          ...
          <div className="flex items-center gap-6 border-l border-white/5 pl-12">
            ...
            <div className="flex items-center gap-4 bg-white/5 px-5 py-2.5 rounded-2xl border border-white/10 hover:border-gold/30 transition-all cursor-pointer group shadow-lg">
              ...
              <div className="text-left hidden md:block">
                <p className="text-[11px] font-black text-white uppercase leading-none group-hover:text-gold transition-colors">Ritik</p>
                <p className="text-[9px] text-gold/40 uppercase mt-1.5 font-bold tracking-tighter">Premium User</p>
              </div>
            </div>
          </div>
        </div>
