...
export default function UserSopPacketsPage() {
  ...
  return (
    <div className="min-h-screen space-y-12 pb-20">
      {/* --- HERO HEADER --- */}
      <div className="relative rounded-[3rem] overflow-hidden bg-card glass-card border border-border p-12 md:p-20 text-center space-y-6">
        ...
        <p className="text-foreground/40 text-sm md:text-lg max-w-2xl mx-auto font-medium leading-relaxed">
      ...
      {/* --- SEARCH & FILTER --- */}
      <div className="flex flex-col md:flex-row gap-6 items-center justify-between sticky top-28 z-40 bg-background backdrop-blur-md p-4 rounded-3xl border border-border">
        <div className="relative flex-1 w-full">
          ...
          <input 
            ...
            className="w-full bg-card shadow-sm border border-border rounded-2xl py-4 pl-12 pr-6 text-foreground font-bold focus:outline-none focus:border-gold/50 transition-all"
          />
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[1,2,3].map(i => (
            <div key={i} className="h-96 bg-card shadow-sm rounded-[2.5rem] animate-pulse"></div>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPackets.map((packet: any, idx) => (
            <motion.div 
              ...
              className="bg-card glass-card rounded-[2.5rem] border border-border overflow-hidden group hover:border-gold/30 transition-all flex flex-col shadow-2xl"
            >
              <div className="aspect-[4/3] bg-card relative overflow-hidden">
                ...
                <div className="absolute top-6 left-6 px-4 py-1.5 bg-foreground/10 backdrop-blur-xl rounded-full text-xs font-black text-gold uppercase tracking-widest border border-gold/20 shadow-xl">
                  {packet.category}
                </div>
              ...
              <div className="p-8 space-y-6 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-xl font-black text-foreground group-hover:text-gold transition-colors">{packet.name}</h3>
                  <p className="text-foreground/40 text-sm mt-3 leading-relaxed line-clamp-3 font-medium">
                ...
                <div className="pt-6 border-t border-border flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-[10px] font-black text-foreground/20 uppercase tracking-[0.2em] mb-1">Packet Price</span>
      ...
      {filteredPackets.length === 0 && !loading && (
        <div className="py-24 text-center space-y-4">
          <div className="w-20 h-20 bg-card shadow-sm rounded-full flex items-center justify-center mx-auto text-foreground/10">
            <Package size={40} />
          </div>
          <h2 className="text-2xl font-black text-foreground/40 uppercase tracking-widest">No matching packets found</h2>
      ...
      <div className="bg-gold/5 border border-gold/10 rounded-[2rem] p-8 flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-foreground font-black text-sm uppercase">Verified Standards</h4>
            <p className="text-foreground/40 text-xs uppercase font-bold tracking-widest">100% Industry Standard SOPs</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-foreground font-black text-sm uppercase">Instant Access</h4>
            <p className="text-foreground/40 text-xs uppercase font-bold tracking-widest">Digital Download available</p>
          </div>
        </div>
      </div>
    </div>
