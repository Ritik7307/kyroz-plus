    ...
    <div className="space-y-12 pb-24 max-w-[1400px] mx-auto relative">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-6 h-full">
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-foreground/40 text-xs mt-2 font-black tracking-[0.2em]">OPEN BILLING INTERFACE</p>
                </div>
              </button>
            </div>
          )}
          ...
          <div className={user?.role === 'cook' ? 'lg:col-span-12 h-full' : 'lg:col-span-6 h-full'}>
            <div className="h-full bg-card glass-card rounded-[2.5rem] p-10 flex flex-col lg:flex-row items-center justify-between gap-8 border border-border shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-3 relative z-10 text-center lg:text-left">
                <h2 className="text-4xl font-black tracking-tighter uppercase leading-none text-foreground">WELCOME BACK, <span className="text-gold">{user?.shopName || user?.name || 'CHEF'}!</span></h2>
              ...
              <div className="flex gap-4 relative z-10 w-full lg:w-auto">
                {(user?.role === 'manager' || user?.role === 'user') && (
                  <div className="flex gap-4">
                    <div className="bg-background p-4 px-6 rounded-2xl border border-border hidden sm:block">
                      <p className="text-[10px] font-black text-foreground/40 uppercase tracking-widest mb-1">Today's Sale</p>
                      <div className="flex items-center gap-3">
                        <span className="text-xl font-black text-foreground">{formatCurrency(dailyRevenue)}</span>
                        <Activity size={14} className="text-blue-500" />
                      </div>
                    </div>
                    <div className="bg-background p-4 px-6 rounded-2xl border border-border hidden sm:block">
                      <p className="text-[10px] font-black text-foreground/40 uppercase tracking-widest mb-1">Daily Gross Profit</p>
                      <div className="flex items-center gap-3">
                        <span className="text-xl font-black text-foreground">{formatCurrency(dailyProfit)}</span>
        ...
        <section className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sops.slice(0, 3).map((sop, idx) => (
                  <motion.div 
                    ...
                    className="bg-card glass-card p-6 rounded-2xl border border-border hover:border-gold/30 cursor-pointer group flex items-center justify-between"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-gold/10 rounded-xl flex items-center justify-center text-gold"><FileText size={24} /></div>
                      <div>
                        <h4 className="font-bold text-sm text-foreground line-clamp-1">{sop.title}</h4>
                        <p className="text-xs text-foreground/40 uppercase tracking-widest mt-0.5">Uploaded {new Date(sop.createdAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <ArrowRight size={18} className="text-foreground/20 group-hover:text-gold transition-colors" />
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-6">
            <h3 className="text-foreground/40 text-[11px] font-black tracking-[0.4em] uppercase">Operational Standards</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                <div key={idx} onClick={() => router.push(item.path)} className="bg-card glass-card p-8 rounded-[2rem] border border-border hover:border-gold/30 cursor-pointer group flex flex-col items-center text-center gap-6 shadow-xl">
                  <div className="w-16 h-16 bg-card shadow-sm rounded-2xl flex items-center justify-center text-foreground/40 group-hover:text-gold group-hover:bg-gold/10 transition-all border border-border">
                    <item.icon size={32} />
                  </div>
                  <div>
                    <h4 className="font-black text-lg uppercase tracking-tight text-foreground">{item.title}</h4>
                    <p className="text-foreground/30 text-xs mt-1 font-bold uppercase tracking-widest">{item.sub}</p>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-4">
          ...
          <div className="lg:col-span-3 space-y-6">
            <h3 className="text-foreground/40 text-[11px] font-black tracking-[0.4em] uppercase">Quick Actions</h3>
            <div className="space-y-3">
              {[
                ...
                <button key={idx} onClick={() => router.push(action.path)} className="w-full bg-card/50 hover:bg-card glass-card p-4 rounded-2xl flex items-center gap-4 group transition-all text-left border border-border">
                  <div className="w-10 h-10 bg-gold-gradient rounded-xl flex items-center justify-center text-black shadow-lg">
                    <action.icon size={20} />
                  </div>
                  <span className="font-black text-[11px] uppercase tracking-widest text-foreground/60 group-hover:text-gold transition-colors">{action.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-9 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {packets.slice(0, 3).map((packet: any, idx) => (
                <div key={idx} className="bg-card glass-card rounded-[2rem] overflow-hidden group border border-border hover:border-gold/30 transition-all shadow-xl flex flex-col">
                  <div className="h-40 overflow-hidden relative bg-card">
                    {packet.images?.[0] ? (
                    ...
                    <div className="absolute top-4 right-4 px-3 py-1 bg-foreground/10 backdrop-blur-md rounded-lg text-[10px] font-black text-gold uppercase tracking-widest border border-border">{packet.category}</div>
                  </div>
                  <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                    <div>
                      <h5 className="font-black text-xs leading-tight text-foreground uppercase tracking-tight line-clamp-2">{packet.name}</h5>
                      <p className="text-xs text-foreground/30 mt-2 line-clamp-2 font-medium">{packet.description || 'Professional commercial collection.'}</p>
                    </div>
                    <div className="flex items-center justify-between pt-4">
                      ...
                      <button 
                        onClick={() => router.push('/dashboard/packets')}
                        className="bg-card shadow-sm hover:bg-gold hover:text-black transition-all px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest border border-border"
                      >
                        Details
                      </button>
                    </div>
              ...
              {packets.length === 0 && (
                <div className="col-span-full py-12 text-center bg-card shadow-sm rounded-[2rem] border border-dashed border-border">
                   <p className="text-foreground/20 font-bold uppercase tracking-widest text-xs">No Premium Packets available yet.</p>
                </div>
              )}
            </div>
          </div>
