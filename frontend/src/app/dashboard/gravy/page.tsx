...
export default function GravyMaster() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-24">
      {/* Header */}
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-10 bg-card/30 p-10 rounded-[3rem] border border-border relative overflow-hidden">
        ...
        <div className="flex gap-4 relative z-10">
           <div className="bg-background p-6 rounded-3xl border border-border flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-gold/10 flex items-center justify-center text-gold"><Clock size={24} /></div>
              <div>
                <p className="text-xs font-black text-foreground/30 uppercase tracking-widest">Active Prep</p>
                <p className="text-xl font-black text-foreground uppercase tracking-tight">Morning Shift</p>
              </div>
           </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4 space-y-6">
          <h3 className="text-foreground/40 text-[11px] font-black tracking-[0.3em] uppercase px-2">Select Base Gravy</h3>
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${
                    activeGravy === g.name ? 'bg-gold text-black shadow-lg' : 'bg-card shadow-sm text-foreground/20 group-hover:text-gold'
                  }`}>
                    <ChefHat size={24} />
                  </div>
                  <div>
                    <h4 className={`font-black text-lg transition-colors ${activeGravy === g.name ? 'text-foreground' : 'text-foreground/40'}`}>{g.name}</h4>
                    <p className="text-xs font-bold text-foreground/20 uppercase tracking-widest">Standard Batch: {g.qtyPerBatch}kg</p>
                  </div>
                </div>
                <ChevronRight size={20} className={activeGravy === g.name ? 'text-gold' : 'text-foreground/5'} />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-card glass-card rounded-[2.5rem] border border-border p-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 relative z-10">
              <div className="space-y-6 flex-1">
                ...
                <div className="space-y-4">
                  <p className="text-[11px] font-black text-foreground/40 uppercase tracking-widest">How many KG of <span className="text-gold">{currentGravy.baseIngredient}</span> are you using?</p>
                  <input 
                    type="number" 
                    value={baseQty}
                    onChange={(e) => setBaseQty(Number(e.target.value))}
                    className="w-full bg-card shadow-sm border border-border rounded-2xl p-6 text-5xl font-black text-gold outline-none focus:border-gold transition-all"
                  />
                </div>
              </div>
              ...
              <div className="bg-gold-gradient p-1 rounded-[2rem] min-w-[280px]">
                <div className="bg-card rounded-[1.9rem] p-8 flex flex-col items-center text-center">
                   ...
                   <div className="w-full h-[1px] bg-card shadow-sm my-6"></div>
                   <p className="text-[10px] font-bold text-foreground/30 uppercase tracking-[0.2em] leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-card glass-card rounded-[2rem] border border-border p-8 flex items-center gap-6">
              ...
              <div>
                <p className="text-xs font-black text-foreground/30 uppercase tracking-widest mb-1">Storage Temperature</p>
                <p className="text-2xl font-black text-foreground">{currentGravy.storageTemp}</p>
              </div>
            </div>
            ...
            <div className="bg-card glass-card rounded-[2rem] border border-border p-8 flex items-center gap-6">
              ...
              <div>
                <p className="text-xs font-black text-foreground/30 uppercase tracking-widest mb-1">Shelf Life (Max)</p>
                <p className="text-2xl font-black text-foreground">{currentGravy.shelfLife}</p>
              </div>
            </div>
          </div>

          <div className="bg-card shadow-sm border border-border rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="space-y-3 flex-1">
              <h4 className="text-lg font-black text-foreground uppercase tracking-tight">Reheating Protocol</h4>
            ...
            <button 
              ...
              className="px-8 py-4 bg-foreground/10 hover:bg-gold hover:text-black rounded-xl text-xs font-black uppercase tracking-widest transition-all"
            >
              View Full SOP
            </button>
          </div>
