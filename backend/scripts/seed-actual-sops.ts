...
async function seed() {
  try {
    ...
    await MasterSop.deleteMany({}); // Clear to avoid duplicates
    await MasterSop.insertMany(actualSops);

    console.log(`Successfully seeded ${actualSops.length} ACTUAL Global SOPs.`);
    process.exit(0);
  } catch (err) {
