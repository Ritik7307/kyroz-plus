...
mongoose.connect('mongodb+srv://vijayshankarprajapati29_db_user:3FxmRRA5ReXi2BqV@cluster0.wf2za1x.mongodb.net/?appName=Cluster0').then(async () => {
  ...
  // Legacy Users
  const pizzas = await Dish.find({ name: /Farmhouse Pizza/i });
  for (const p of pizzas) {
    const r = await Recipe.findOne({ targetModel: 'Dish', targetId: p._id });
    if (!r) continue;
    
    let newQuantity = 0;
    if (p.name === 'Personal Farmhouse Pizza') newQuantity = 40;
    else if (p.name === 'Medium Farmhouse Pizza') newQuantity = 60;
    else if (p.name === 'Large Farmhouse Pizza') newQuantity = 100;
  ...
  const portions = await SemiFinishedGood.find({ name: /Veg Portion/i });
  ...
  console.log(`Successfully updated ${updatedPizzaCount} Farmhouse Pizza/Veg Portion recipes.`);
  mongoose.disconnect();
});
