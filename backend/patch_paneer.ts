...
mongoose.connect('mongodb+srv://vijayshankarprajapati29_db_user:3FxmRRA5ReXi2BqV@cluster0.wf2za1x.mongodb.net/?appName=Cluster0').then(async () => {
  ...
  // Update all Paneer Pizza recipes to match the raw paneer requirement
  // 100g raw = 100g SFG_PANEER_TOPPING, 60g raw = 60g SFG_PANEER_TOPPING, 40g raw = 40g SFG_PANEER_TOPPING
  
  // Legacy Users
  const pizzas = await Dish.find({ name: /Paneer Pizza/i });
  for (const p of pizzas) {
    const r = await Recipe.findOne({ targetModel: 'Dish', targetId: p._id });
    if (!r) continue;
    
    let newQuantity = 0;
    if (p.name === 'Personal Paneer Pizza') newQuantity = 40;
    else if (p.name === 'Medium Paneer Pizza') newQuantity = 60;
    else if (p.name === 'Large Paneer Pizza') newQuantity = 100;
    ...
    for (let ing of r.ingredients) {
      if (ing.itemModel === 'SemiFinishedGood') {
        ...
        if (sfg && sfg.name.includes('Paneer Topping')) {
  ...
  const portions = await SemiFinishedGood.find({ name: /Paneer Portion/i });
  ...
  console.log(`Successfully updated ${updatedPizzaCount} Paneer Pizza/Portion recipes.`);
  mongoose.disconnect();
});
