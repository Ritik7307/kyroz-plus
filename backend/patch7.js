...
function injectAtRegex(regex, injectionStr) {
  if (regex.test(content)) {
    content = content.replace(regex, injectionStr + '\\n$1');
    return true;
  }
  return false;
}

const rmInjection = `    // Indian Gravies 3
...
if (!content.includes('RM_G203')) {
  console.log('RM Inject:', injectAtRegex(/(\\s*\\];\\s*const rmIds)/, rmInjection));
}

const pkgInjection = `    // Copper Kadhai / Oval Plate
...
if (!content.includes('PKG_COPPER_KADHAI')) {
  console.log('PKG Inject:', injectAtRegex(/(\\s*\\];\\s*const pkgIds)/, pkgInjection));
}

const sfgInjection = `    // Hariyali and Kadhai
...
if (!content.includes('SFG_G203')) {
  console.log('SFG Inject:', injectAtRegex(/(\\s*\\];\\s*const sfgIds)/, sfgInjection));
}

const sfgRecipeInjection = `    'SFG_G203': [
...
if (!content.includes("'SFG_G203':")) {
  console.log('SFG Recipe Inject:', injectAtRegex(/(\\s*\\};\\s*for \\(const sfgCode of Object\\.keys\\(sfgRecipeMappings\\)\\))/, sfgRecipeInjection));
}

const dishInjection = `    { name: 'Chicken Tikka Masala', price: 380, category: 'Main Course', packagingLogic: { takeaway: [pkgIds['PKG_EARTHEN_HANDI'], pkgIds['PKG_LID'], pkgIds['PKG_CARRY_BAG']], delivery: [pkgIds['PKG_EARTHEN_HANDI'], pkgIds['PKG_LID'], pkgIds['PKG_CARRY_BAG']], dineIn: [pkgIds['PKG_EARTHEN_HANDI']] } },
...
if (!content.includes('Chicken Tikka Masala')) {
  console.log('Dish Inject:', injectAtRegex(/(\\s*\\];\\s*const recipeMappings)/, dishInjection));
}
...
if (!content.includes("'Chicken Tikka Masala':")) {
  console.log('Dish Recipe Inject:', injectAtRegex(/(\\s*\\};\\s*for \\(const dish of dishData\\))/, dishRecipeInjection));
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('Patch 7 applied successfully!');
